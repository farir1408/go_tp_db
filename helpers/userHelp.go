...
const SelectUsersSinceDesc =  `SELECT about, email, fullname, nickname FROM forum_users
								WHERE forum_slug = $1 AND nickname < $2
								ORDER BY nickname DESC
								LIMIT $3::TEXT::INTEGER`

const SelectUsersSince = `SELECT about, email, fullname, nickname FROM forum_users
								WHERE forum_slug = $1 AND nickname > $2
								ORDER BY nickname
								LIMIT $3::TEXT::INTEGER`

const SelectUsers = `SELECT about, email, fullname, nickname FROM forum_users
								WHERE forum_slug = $1
								ORDER BY nickname
								LIMIT $2::TEXT::INTEGER`

const SelectUsersDesc = `SELECT about, email, fullname, nickname FROM forum_users
								WHERE forum_slug = $1
								ORDER BY nickname DESC
								LIMIT $2::TEXT::INTEGER`
